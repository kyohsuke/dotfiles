...
if [ "$CHANGE_OSX_DEFAULTS" == "true" ]; then
...
# Set the computer to sleep after 180 minutes
sudo systemsetup -setcomputersleep 180

# Set the display to sleep after 15 minutes
sudo systemsetup -setdisplaysleep 15

# Never go into disk sleep mode
...
# Change position on screen to the left
...
hash tmutil &> /dev/null && tmutil --help | grep -q disablelocal && sudo tmutil disablelocal
